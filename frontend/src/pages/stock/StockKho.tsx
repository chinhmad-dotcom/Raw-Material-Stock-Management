...
const StockKho: React.FC = () => {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2 overflow-y-auto px-2 py-2 text-slate-900 dark:text-slate-100">
      ...
      {/* Header */}
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 p-3  shadow-sm min-h-[60px] relative z-50">
      ...
      <div className="flex flex-col gap-1">
        {ZONES.map((zone, idx) => (
          <section key={idx} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white/50 dark:bg-slate-900/50 p-2 sm:p-3">
            <div className="mb-2 flex items-center gap-2 border-b border-slate-200 dark:border-white/10 pb-2">
              <div className="h-3 w-3 rounded bg-sky-500/40 border border-sky-500/50"></div>
              <h3 className="text-sm font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400">{t('pages.stockKho.zones.' + idx, zone.name)}</h3>
            </div>
                  ...
                  <div key={loc} className="flex min-h-[60px] flex-col rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 p-1.5 shadow-inner hover:border-sky-500/50 transition-colors">
                    ...
                    {items.length > 0 ? (
                      <div className="grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] gap-x-2 gap-y-1">
                        {items.map(item => (
                          <div key={item.materialId} className="flex flex-col mb-0.5">
                            <span className="truncate text-[10px] font-semibold text-slate-800 dark:text-slate-200" title={item.materialName}>{item.materialName}</span>
                            <span className="text-xs font-mono font-bold text-sky-600 dark:text-sky-400 leading-none">{(item.currentStockTons * 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })} kg</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="flex flex-1 items-center justify-center">
                        <span className="text-[10px] text-slate-700">{t('pages.stockKho.empty', 'Trống')}</span>
                      </div>
                    )}
                  </div>
                );
        ...
        <section className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white/50 dark:bg-slate-900/50 p-5">
            <div className="mb-4 flex items-center gap-2 border-b border-slate-200 dark:border-white/10 pb-3">
              <div className="h-4 w-4 rounded bg-slate-500/40 border border-slate-500/50"></div>
              <h3 className="text-lg font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">{t('pages.stockKho.otherLocations', 'CÁC VỊ TRÍ KHÁC')}</h3>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10">
              {additives
                ...
                .map(item => (
                  <div key={item.materialId} className="flex min-h-[90px] flex-col rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 p-3 shadow-inner">
                    <div className="mb-1 text-xs font-black text-slate-500 dark:text-slate-500">{item.warehouseLocation || 'N/A'}</div>
                    <div className="flex flex-col">
                      <span className="truncate text-xs font-semibold text-slate-800 dark:text-slate-200" title={item.materialName}>{item.materialName}</span>
                      <span className="text-sm font-mono font-bold text-sky-600 dark:text-sky-400">{(item.currentStockTons * 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })} kg</span>
                    </div>
                  </div>
              ))}
            </div>
