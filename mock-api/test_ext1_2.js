...
(async () => {
    ...
    await page.evaluate(() => {
      document.querySelector('input[name="TextBox2"]').value = '19 September 2026';
      document.querySelector('input[name="FromhY"]').value = '2026';
      document.querySelector('input[name="FromhM"]').value = '9';
      document.querySelector('input[name="FromhD"]').value = '19';
      
      document.querySelector('input[name="TextBox3"]').value = '20 September 2026';
      document.querySelector('input[name="TohY"]').value = '2026';
      document.querySelector('input[name="TohM"]').value = '9';
      document.querySelector('input[name="TohD"]').value = '20';
    });
    ...
    if(pages.length > 1) {
        ...
        const data = await newPage.evaluate(() => {
          ...
          return rows.map(tr => Array.from(tr.querySelectorAll('td, th')).map(td => td.innerText.trim())).filter(row => row[0] === 'METER' || row[0] === 'Meter' || row[0] === 'EXT1');
        });
